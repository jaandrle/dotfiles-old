...
Options:
  -c --create-book                Create a basic skeleton of
                                  an ebook in this folder.
  -d --create-book-advanced       Create an advanced skeleton
                                  of an ebook in this folder.
  -m --generate-mobi              (Requires Calibre) Convert the
                                  compiled EPUB file to the MOBI
                                  format for use with Amazon Kindle.
  -o --output-folder=<folder>     Specify an alternative location
                                  for the generated ebook.
EOF
  exit 1
}
...
#publisher:  My Press
#rights:  (c) 2007 John Smith, CC BY-NC" > ./$METADATA_FILE
    echo "body{font-family:serif;text-align: justify;}" > stylesheet.css
}

create_book_project_advanced () {
    mkdir -p ./$IMAGE_FOLDER
    mkdir -p ./$FONT_FOLDER
    echo "For cover image: ./bookcover.jpg"
}

# available options
OPTS=`getopt -o cdmo: -l create-book,create-book-advanced,generate-mobi,output-folder: -- "$@"`
