...
"" #region COC – COC and so on, compilers, code/commands completions
	...
	command! -range -nargs=? AIcommit <line1>,<line2>call AIRun('I want you to act like a git commit message writer. I will input a git diff and your job is to convert it into a useful commit message. Return nothing else but only ONE commit message without quotes or backticks. Return a concise, precise, present-tense, complete sentence. The length should be fewer than 50 characters if possible.'.<f-args>)
	command! -range -nargs=? AIcommitConventional <line1>,<line2>AIcommit 'It should follow the conventional commits. The format is <type>: <description>. A type can be one of the following: build, chore, ci, docs, feat, fix, perf, refactor, revert, style, or test. Use it to precisely describe the change.' <q-args>
